...
void generate_codes(HuffNode *root, HuffCode *table, unsigned long long code, unsigned char len) {
    ...
    if (root->left) {
        generate_codes(root->left, table, code << 1, len + 1);
    }
    if (root->right) {
        generate_codes(root->right, table, (code << 1) | 1, len + 1);
    }
}
...
void serialize_tree(HuffNode *root, unsigned char **buffer, unsigned long *size) {
    ...
    if (!root->left && !root->right) {
        ...
        *buffer = malloc(*size);
        (*buffer)[0] = 0;
        (*buffer)[1] = root->byte;
        if (left_size > 0) memcpy(*buffer + 2, left_buf, left_size);
        if (right_size > 0) memcpy(*buffer + 2 + left_size, right_buf, right_size);
    } else {
        *size = 1 + left_size + right_size;
        *buffer = malloc(*size);
        (*buffer)[0] = 1;
        if (left_size > 0) memcpy(*buffer + 1, left_buf, left_size);
        if (right_size > 0) memcpy(*buffer + 1 + left_size, right_buf, right_size);
    }
...
unsigned char* decompress_data(const unsigned char *compressed, unsigned long comp_size,
                                HuffNode *root, unsigned long *out_size) {
    if (!root) return NULL;
    
    unsigned long size_estimate = comp_size * 8;
    unsigned char *output = malloc(size_estimate);
    unsigned long output_pos = 0;
    
    unsigned long bit_pos = 0;
    HuffNode *current = root;
    
    for (unsigned long i = 0; i < comp_size && bit_pos < comp_size * 8; i++) {
        unsigned char byte = compressed[i];
        
        for (int bit = 7; bit >= 0 && bit_pos < comp_size * 8; bit--) {
            ...
            if (!current->left && !current->right) {
                if (output_pos >= size_estimate) {
                    size_estimate *= 2;
                    output = realloc(output, size_estimate);
                }
